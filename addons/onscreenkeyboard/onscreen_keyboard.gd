...
func _input(event):
	...
	# Listen for physical hardware keyboard presses
	if event is InputEventKey and not event.is_echo():
		for key_button in keys:
			# Check if this button matches the physical key pressed
			if key_button.has_meta("physical_keycode") and key_button.get_meta("physical_keycode") == event.keycode:
	...
	for layout in data.get("layouts"):
		...
		for row in layout.get("rows"):
			...
			for key in row.get("keys"):
				...
				if key.has("output"):
					var code = KeyListHandler.get_key_from_string(key.get("output"))
					new_key.set_meta("physical_keycode", code)
